...
void Test_Root_Geometry()
{
  ...
  TGeoMaterial *mat_14 = new TGeoMaterial( "mat_14",28,14,-2.3290 );
  TGeoMedium   *med_14 = new TGeoMedium( "med_14",2,mat_14 );
  ...
  TGeoVolume *cube = gGeoManager->MakeBox( "CUBE",void_mat,2.55,2.55,2.0 );
  cube->SetUniqueID(1);
  cube->SetVisibility(kTRUE);

  // Define the slabe volume
  TGeoVolume *slabe = gGeoManager->MakeBox( "CUBE",med_14,2.5,2.5,0.0002 );
  slabe->SetUniqueID(2);

  // Add the slabe as a daughter of the cube
  terminal_cube->AddNode( cube, 1 );
  cube->AddNode( slabe, 1 );

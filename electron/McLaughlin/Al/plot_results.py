...
plt.title('$\mathrm{Energy\/Deposition\/from\/3\/MeV\/Electron\/in\/Aluminum}$', size=16)
ax=plt.gca()
...
if user_args.e:
    ...
    # Get experimental data for the dye film
    filename = directory + "/dye_film_results.tsv"
    ...
    # plt1, = plt.scatter(exp_x, exp_y, label="McLaughlin (Dye Film)", marker='s' )
    plt1, = plt.plot(exp_x, exp_y, linestyle='None', marker='s', markersize=5, color='b' )

    # Get experimental data for the ionization chamber
    filename = directory + "/ionization_chamber_results.tsv"
    with open(filename) as input:
        data = zip(*(line.strip().split('\t') for line in input))
        exp_x = np.asfarray(data[0][1:])
        exp_y = np.asfarray(data[1][1:])
    ...
    # Plot the experimental data
    # plt2, = plt.scatter(exp_x, exp_y, label="McLaughlin (Ion. Chamber)", marker='o' )
    plt2, = plt.plot(exp_x, exp_y, linestyle='None', marker='o', markersize=5, color='b' )

    plots.append( plt1 )
    labels.append("McLaughlin (Dye Film)" )
    plots.append( plt2 )
    labels.append("McLaughlin (Ion. Chamber)" )
